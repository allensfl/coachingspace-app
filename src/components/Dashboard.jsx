...
import {
  Users, Calendar, Target, Plus, Check, Edit, Trash2, Send, User,
  AlertTriangle, CheckCircle, Clock, CalendarDays, Settings, DollarSign,
  TrendingUp, BookOpen, Download, Upload, Share2
} from 'lucide-react';
...
const Dashboard = () => {
  ...
  const loadTasksFromSupabase = async () => {
    try {
      ...
      const allTasks = [...supabaseTasks];
      
      localTasks.forEach(localTask => {
        const existsInSupabase = supabaseTasks.some(st => st.title === localTask.title);
        if (!existsInSupabase && localTask.dueDate) {
          allTasks.push({
            id: localTask.id,
            ...
            coachee_id: localTask.assignedTo,
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto">
                {coacheeDeadlines.length === 0 ? (
                  <div className="text-center py-8 text-slate-400">
                    ...
                    <p className="text-sm text-slate-500 mt-1">Erstelle Tasks bei den Coachees</p>
                  </div>
                ) : (
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              >
                ...
                <span className="text-sm text-slate-300">Task für Coachee</span>
              </button>
            ...
            <DialogContent className={classes.card + " border-slate-700 max-w-2xl"}>
              ...
              <div className="space-y-4">
                ...
                <div className="flex space-x-3">
                  {selectedSharedItem.type === 'document' && (
                    <Button
                      onClick={() => {
                        const coachee = coachees.find(c => c.id === selectedSharedItem.coacheeId);
                        const docs = coachee?.portalData?.documents || [];
                        const docId = selectedSharedItem.id.replace('doc_', '');
