...
const InvoiceDialog = ({ open, onOpenChange, onSave, coachees, recurringInvoices, invoice, sessions }) => {
    ...
    const [taxRate, setTaxRate] = useState(0); // NEU
    const [showSessionDialog, setShowSessionDialog] = useState(false);
    const [showPreview, setShowPreview] = useState(false); // NEU
    ...
    React.useEffect(() => {
        if (invoice) {
            ...
            setTaxRate(invoice.taxRate || 0); // NEU
        } else {
            setCoacheeId('');
            ...
            <Dialog open={open} onOpenChange={onOpenChange}>
                <DialogContent className="bg-gray-800 border-gray-700 text-white max-w-4xl max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="space-y-6 py-4">
                        ...
                        {/* NEU: Preview */}
                        {showPreview && items.filter(i => i.description.trim()).length > 0 && (
                                    ...
                                    <div key={invoice.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 bg-gray-700 rounded-lg hover:bg-gray-650 transition-colors gap-4">
                                        <div className="flex items-center gap-4 flex-1">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <p className="text-sm text-gray-400 mt-1">{invoice.coacheeName}</p>
