...
import { Plus, Search, User, Mail, ShieldCheck, ShieldOff, ClipboardCopy, Send, Calendar, Trash2, AlertTriangle } from 'lucide-react';
...
const NewCoacheeDialog = ({ open, onOpenChange, addCoachee, customFields }) => {
  ...
  const handleChange = (e) => {
    ...
    console.log(`Form-Feld geändert: ${id} = ${value}`);
  ...
  const handleSubmit = (e) => {
    ...
    console.log('Neuer Coachee wird erstellt:', newCoacheeData);
...
export default function Coachees() {
  ...
  // NEU: Coachee löschen mit allen verknüpften Daten
  const handleDeleteCoachee = async (coacheeId) => {
    ...
    try {
      // 1. Tasks aus localStorage löschen
      ...
      console.log('Lokale Tasks gelöscht');

      // 2. Supabase pushed_tasks löschen
      try {
        ...
        if (userData?.user) {
          ...
          console.log('Supabase Tasks gelöscht');
        }
      } catch (supabaseError) {
      ...
      // 5. Coachee aus AppStateContext entfernen
                   ...
                   <Link to={`/coachees/${coachee.id}`} className="block">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {/* NEU: Löschen-Button in der Card */}
                          <AlertDialog>
