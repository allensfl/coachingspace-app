...
const useAppState = () => {
  ...
  const updateCoacheeConsent = useCallback(async (coacheeId, consentKey, value, pdfBlob) => {
    console.log('updateCoacheeConsent called:', { coacheeId, consentKey, value, hasPdfBlob: !!pdfBlob });
    
    // DSGVO PDF-Archivierung mit IndexedDB - VOR dem State Update
    if (consentKey === 'gdpr' && value && pdfBlob) {
      ...
      if (coachee) {
        ...
        reader.onload = async (e) => {
          try {
            ...
            // PDF in IndexedDB speichern
            await documentStorage.storeDocument(consentDocument);
            console.log('PDF stored in IndexedDB:', consentDocument.name);
            
            // Metadaten für localStorage
            const documentMetadata = {
              id: documentId,
              name: consentDocument.name,
              type: 'contract',
              uploadDate: consentDocument.uploadDate,
              size: consentDocument.size,
              format: 'PDF',
              coacheeId: coachee.id,
            ...
            // EINZIGER State-Update: Consent + PDF + AuditLog zusammen
            setCoachees(currentCoachees => currentCoachees.map(c => {
              if (c.id === parseInt(coacheeId)) {
                const updated = {
                  ...
                  // PDF hinzufügen
                  documents: [...(c.documents || []), documentMetadata],
                  // AuditLog erweitern
                  auditLog: [
                    ...(c.auditLog || []), 
                    { 
                    ...
                    { 
                      timestamp: new Date().toISOString(), 
                      user: 'System', 
                      action: 'DSGVO-Einwilligungs-PDF in IndexedDB archiviert' 
                    }
                  ],
                };
                console.log('Combined update - Consent:', updated.consents.gdpr, 'Documents:', updated.documents.length);
                return updated;
              }
              return c;
            }));
            ...
            toast({
              title: "PDF-Speicherung fehlgeschlagen",
      ...
      // Für andere Consents ohne PDF - normaler Update
  ...
  // Geteilte Journal-Einträge von Coachee-Portalen laden
  const getSharedJournalEntries = useCallback(() => {
    if (!coachees) return [];
    ...
    coachees.forEach(coachee => {
      try {
        // Portal-Daten aus localStorage laden
        const portalDataKey = `coacheePortalData_${coachee.id}`;
        const portalDataString = localStorage.getItem(portalDataKey);
        
        if (portalDataString) {
          const portalData = JSON.parse(portalDataString);
          const coacheeJournalEntries = portalData.journalEntries || [];
          
          // Nur geteilte Einträge sammeln
          const coacheeSharedEntries = coacheeJournalEntries
            .filter(entry => entry.isShared === true)
            .map(entry => ({
              ...entry,
              // Markierung für Coach-Ansicht
              id: `shared-${coachee.id}-${entry.id}`,
              isShared: true,
              coacheeId: coachee.id,
              sharedBy: `${coachee.firstName} ${coachee.lastName}`,
              readOnly: true,
              syncedAt: new Date().toISOString()
            }));
          
          sharedEntries.push(...coacheeSharedEntries);
        }
      } catch (error) {
        console.error(`Error loading portal data for coachee ${coachee.id}:`, error);
      }
    });
  ...
  // Alle Journal-Einträge: Eigene + Geteilte von Coachees
  const getAllJournalEntries = useCallback(() => {
    const ownEntries = journalEntries || [];
    const sharedEntries = getSharedJournalEntries();
  ...
  }, [journalEntries, getSharedJournalEntries]);
  ...
  const contextValue = useMemo(() => ({
    ...
    // Action functions - ALLE exportieren
    ...
    activatePackage, getSharedJournalEntries, getAllJournalEntries,
    ...
    addCoachee, updateCoachee, getCoacheeById, getCoacheeByToken, ensurePermanentTokenForDemo,
    getToolById, updateCoacheeConsent, getAllCoacheeDocuments, addDocument, updateSettings, backupData, deductFromPackage, updateJournalCategories, addToolUsage, updateToolCategories,
    activatePackage, getSharedJournalEntries, getAllJournalEntries, addTask, updateTask, deleteTask
  ]);
